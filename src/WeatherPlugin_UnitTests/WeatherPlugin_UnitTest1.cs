...
public class Tests
{
    ...
    public async Task Given_InvalidKey_Then_ThrowsException()
    {
        ...
        Assert.Throws<Exception>(() => api.GetWeatherDataByCityNameAsync("London").GetAwaiter().GetResult());
    ...
    public async Task Given_CountryName_Then_ReturnsWeatherData()
    {
        ...
        Assert.AreEqual(weatherData?.City, "London");
        Assert.AreEqual(weatherData?.Country, "GB");
    }
